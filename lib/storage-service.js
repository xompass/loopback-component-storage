...
var storage = require('@xompass/pkgcloud').storage;
 ...
 * @promise
 */
StorageService.prototype.getSignedUrl = function (container, file, req, res, cb) {
  cb = cb || utils.createPromiseCallback();
  if (handler.getSignedUrl) {
    handler.getSignedUrl(this.client, req, res, container, file, cb);
    return cb.promise;
  }

  cb(new Error('getSignedUrl not implemented'));
};
