...
var pkgcloud = require('@xompass/pkgcloud');
...
/*!
 * Patch the @xompass/pkgcloud Container/File classes so that the metadata are separately
  ...
  try {
    ...
    try {
      ...
      if (!handler || !handler.createClient) {
        // Fall back to @xompass/pkgcloud
        handler = require('@xompass/pkgcloud').storage;
      }
    } catch (err) {
      // Fall back to @xompass/pkgcloud
      handler = require('@xompass/pkgcloud').storage;
...
function getProvider(provider) {
  try {
    ...
    try {
      ...
      // Fall back to @xompass/pkgcloud
      return require('@xompass/pkgcloud').providers[provider];
    }
  }
}
