    ...
    if (part.filename === undefined || part.filename === '') {
      ...
      part.on('data', function (buffer) {
        ...
        if (self._fieldsSize > self.maxFieldsSize) {
          ...
          error.limit = self.maxFieldsSize;
 ...
 * @param {Boolean} [signedUrl] returns a signed url instead of download the file
 * @callback {Function} cb Callback function.
...
exports.download = function (provider, req, res, container, file, cb) {
  ...
  let signedUrl = req.query.signedUrl || req.query['signed-url'];

  const signedUrlConfig = provider.config?.signedUrl;
  if (signedUrl && signedUrlConfig.enabled) {
    const maxAge = signedUrlConfig.expiresIn * 0.9;
    res.set('Cache-Control', `public, max-age=${maxAge}`);
    res.redirect(getSignedUrl(provider, params.container, params.remote));
    return cb(null);
  }
  ...
  function setupPartialDownload(params, stats, res) {
    ...
    res.set(
      'Content-Range',
      'bytes ' + params.start + '-' + params.end + '/' + total,
    );
...
exports.getSignedUrl = function (provider, req, res, container, file, cb) {
  container = container || (req && req.params.container);
  file = file || (req && req.params.file);
  ...
  const signedUrl = getSignedUrl(provider, container, file);

  cb(null, signedUrl ? { url: signedUrl } : null);
};

function getSignedUrl(provider, container, file) {
  try {
...
function processError(err, fileName) {
  if (err.code === 'ENOENT' || err.code === 'NoSuchKey') {
    ...
    // Remove stack trace for 404 errors to avoid verbose logging
    delete err.stack;
  }
  return err;
}
